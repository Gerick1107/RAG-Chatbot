...
def module_hr_faq():
    ...
    with tab_faq:
        ...
        st.caption("Try:  How many casual leaves do I have?  •  What is the WFH policy?  •  What documents are needed for reimbursement?")
    ...
    with tab_gen:
        ...
        if st.button("Generate", type="primary"):
            ...
            context="\n\n".join(hits)
            instructions={
                "First-week checklist":"Create a clear, friendly day-by-day first-week checklist (Day 1 to Day 5) as markdown with checkboxes.",
                "Joining email draft":"Write a warm welcome/joining email from HR to the new joiner with key Day-1 details.",
                "Orientation schedule":"Create a Day-1 orientation schedule as a markdown table with time slots and sessions.",
            }[what]
            prompt=(
                f"You are an HR assistant. {instructions} The new joiner's role is '{role}'. "
                "Use the company context below where relevant; if a detail isn't there, use sensible generic defaults.\n\n"
                f"COMPANY CONTEXT:\n{context or '(none available)'}"
            )
            with st.spinner("Generating..."):
                st.markdown(chat([{"role":"user","content":prompt}], temperature=0.4, max_tokens=900))
...
def screen_resume(jd, resume_text):
    ...
    return parse_json(chat([{"role":"user","content":prompt}], temperature=0.2, max_tokens=900))
...
def module_resume_screening():
    ...
    jd=st.text_area("Job Description", height=180, placeholder="Paste the full job description here...")
    files=st.file_uploader("Upload resume PDFs", type=["pdf","txt"], accept_multiple_files=True)
    if st.button("Screen resumes", type="primary"):
        if not jd.strip():
            st.warning("Please paste a job description first."); return
        if not files:
            st.warning("Please upload at least one resume."); return
        results=[]
        progress=st.progress(0.0)
        for n,f in enumerate(files,1):
            with st.spinner(f"Analysing {f.name}..."):
                data=screen_resume(jd, read_uploaded(f))
            if data:
                data["_file"]=f.name; results.append(data)
            else:
...
def draft_leave(request_text):
    prompt=(
        ...
        "details and draft a formal leave-request email. Respond with ONLY a JSON object of this shape:\n"
        "{\n"
    ...
    return parse_json(chat([{"role":"user","content":prompt}], temperature=0.3, max_tokens=700))
...
def module_leave_request():
    ...
    if st.button("Draft request", type="primary"):
        if not req.strip():
            st.warning("Please describe your leave request first."); return
        with st.spinner("Drafting..."):
            data=draft_leave(req)
        if not data:
            st.error("Couldn't draft the request. Please try rephrasing."); return
    ...
    to_email=st.text_input("Send to (manager or any recipient)",
                           value=get_manager_email(), placeholder="recipient@company.com")
    sender_name=st.text_input("Your name (for the From field)", value="")
    if not smtp_configured():
        st.info("📭 SMTP isn't configured — copy the draft above. "
                "Add `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` to `.env` to enable sending.")
...
def module_interview_questions():
    ...
    resume_file=st.file_uploader(
        "Upload candidate's resume (optional — adds resume-specific questions)",
        type=["pdf","txt"])
    if st.button("Generate questions", type="primary"):
        if not role.strip():
            st.warning("Please enter a role."); return
        with st.spinner("Generating general questions..."):
            st.markdown(chat([{"role":"user","content":(
                f"Generate {count} interview questions for HR to ask a {level}-level '{role}' candidate. "
                "Format as a numbered markdown list grouped under **Technical**, **Problem-solving**, and **Behavioural** headings."
        ...
        if resume_file:
            with st.spinner("Generating resume-specific questions..."):
                st.markdown(chat([{"role":"user","content":(
                    ...
                    "Based on the resume below, generate 5 targeted questions probing specific experiences, "
                    "projects, or skills mentioned. Format as a numbered markdown list under a **Resume-specific** heading.\n\n"
                    f"RESUME:\n{read_uploaded(resume_file)}"
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Drop PDFs into `company_docs/` and the HR FAQ module picks them up.")
    if smtp_configured(): st.success("SMTP configured ✓")
    else: st.warning("SMTP not configured")
